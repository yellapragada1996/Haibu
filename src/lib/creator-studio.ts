...
export async function getCreatorEarnings(profileId: string) {
  ...
  const doneStatuses = sql`(${bookings.status} IN ('completed', 'no_show_fan') OR (${bookings.status} = 'cancelled_fan' AND COALESCE(${bookings.effective_payout_cents}, ${bookings.creator_payout_cents}) > 0) OR (${bookings.status} = 'no_show_creator' AND ${bookings.effective_payout_cents} IS NOT NULL AND ${bookings.effective_payout_cents} > 0))`;
